...
class MultiTrajRouterNode:
    ...
    def handle_traj_following(self, participants):
        """轨迹跟随命令"""
        success = []
        fail = []
        ...
        for uav_id in participants:
            ...
            if state.current_state == state.STATE_HOVER:
                self.request_trajectory_from_downstream(state)
                success.append(uav_id)
            else:
                fail.append(f"{uav_id}({state.current_state})")
        ...
        if fail:
            return JsonCommandResponse(False, f"失败: {fail}")
        return JsonCommandResponse(True, f"轨迹跟随: {success}")
